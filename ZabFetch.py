...
class ZabbixDB:
    ...
    def get_alerts(self, time_from: int = None, time_to: int = None, hostname: str = None, limit: int = None, host_group: str = None,severity: list[str] = None,status: list[str] = None,item_name:List[str] = None):
        ...
        try:
            ...
            if item_name is not None:
                if not isinstance(item_name, list):
                    logger.warning("Item name must be a list of item names")
                alerts = alerts[alerts['item_name'].isin(item_name)]
            if hostname is not None:
                alerts = alerts[alerts['host'] == hostname]
            if severity is not None:
                if not isinstance(severity, list):
                    raise ZabbixDBError("Severity must be a list of severity levels")
            ...
            if status is not None:
                valid_statuses = set(status) & {'resolved', 'problem'}

                if not valid_statuses:
                    logger.error(f"Invalid status values: {status}")
                else:
                    filters = []
                    if 'resolved' in valid_statuses:
                        filters.append(alerts['recovery_eventid'].notnull())
                    if 'problem' in valid_statuses:
                        filters.append(alerts['recovery_eventid'].isnull())

                    if filters:
                        combined_filter = filters[0]
                        for f in filters[1:]:
                            combined_filter |= f  # OR condition
                        alerts = alerts[combined_filter]

            if alerts.empty:
                logger.warning("No alerts found after filtering")
                return []
            else:
                alerts.drop(columns=['item_name'], inplace=True, errors='ignore')
