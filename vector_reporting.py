...
def format_uptime(seconds):
    ...
    parts = []
    if years > 0:
        parts.append(f"{years:02}y")
    if month > 0 or years > 0:
        parts.append(f"{month:02}m")
    if day > 0 or month > 0 or years > 0:
        parts.append(f"{day:02}d")
    parts.append(f"{hour:02}h")
    parts.append(f"{minute:02}m")
    parts.append(f"{sec:02}s")
    
    return ' '.join(parts)

def calculate_uptime(data):
    # Ensure data is sorted by timestamp
    data = data.sort_values(by='clock').reset_index(drop=True)
    
    # Calculate total observation period
    time_from = data['clock'].min()
    time_to = data['clock'].max()
    total_time = time_to - time_from  # Total duration in seconds
    
    # Initialize variables
    total_uptime = 0
    prev_value = 0
    prev_clock = data.loc[0, 'clock']
    max_int32 = 2**32  # Handle 32-bit counter rollover (seconds)

    
    # Check for reboots
    reboots_detected = any(data['value'][i] < data['value'][i-1] for i in range(1, len(data)))
    
    if not reboots_detected:
        # No reboots: uptime equals total observation time
        total_uptime = total_time
    else:
        # Calculate uptime with reboot handling
        for i in range(len(data)):
            current_value = data.loc[i, 'value']
            current_clock = data.loc[i, 'clock']
            
            # Calculate time delta between polls
            time_delta = current_clock - prev_clock if i > 0 else 30
            
            # Handle potential counter rollover or reboot
            if i > 0 and current_value < prev_value:
                # Check for rollover
                if prev_value > max_int32 - 30 and current_value < 30:
                    value_diff = current_value + (max_int32 - prev_value)
                else:
                    value_diff = current_value
            else:
                # No reboot: tolerate 1-second shortfall
                value_diff = current_value - prev_value if i > 0 else current_value
                uptime_increment = time_delta if value_diff >= time_delta - 1 else min(value_diff, time_delta)
            
            total_uptime += uptime_increment
            
            
            prev_value = current_value
            prev_clock = current_clock
    
    # Ensure uptime does not exceed total_time
    total_uptime = min(total_uptime, total_time)
    
    # Calculate downtime and availability
    downtime = max(0, total_time - total_uptime)
    availability = round((total_uptime / total_time) * 100, 4) if total_time > 0 else 0
    
    # Report system uptime from max value
    system_uptime = data['value'].max()    
    return format_uptime(total_uptime), format_uptime(downtime), availability, format_uptime(system_uptime)
...
def get_overview_report(
    ...
    group: list[str],
    hostname: list[str],
    client_group: list[str] = None
) -> dict[str, pd.DataFrame]:
    
    if report_type != 'Avalibility':
        return {}

    # Mapping of group types to their uptime metric names
    uptime_items = {
        "Linux servers": "System uptime",
        "server": "Uptime",
        "Zabbix servers": "System uptime",
        "switch": "Uptime (network)",
        "Switches": "Uptime (network)",
        "Windows_servers": "Uptime",
        "Firewall":"System uptime"
    }

    grouped_hosts = {}
    for grp in group:
        hosts_in_group = zbx.get_host_by_group(host_group=grp)
        grouped_hosts[grp] = [h['host_name'] for h in hosts_in_group]

    # Apply client_group and hostname filtering
    if client_group:
        client_hosts_raw = zbx.get_host_by_group(host_group=client_group)
        client_hosts = {h['host_name'] for h in client_hosts_raw}

        filtered_hosts = {}
        for grp, hosts in grouped_hosts.items():
            intersected = set(hosts) & client_hosts
            if hostname != ['all']:
                intersected &= set(hostname)
            if intersected:
                filtered_hosts[grp] = list(intersected)

        grouped_hosts = filtered_hosts
 
    elif hostname != ['all']:
        # If only hostname filtering is needed
        filtered_hosts = {}
        for grp, hosts in grouped_hosts.items():
            intersected = set(hosts) & set(hostname)
            if intersected:
                filtered_hosts[grp] = list(intersected)
        grouped_hosts = filtered_hosts

    # Fetch uptime and compile reports
    grouped_uptime_data = {}
    for grp, host_list in grouped_hosts.items():
        uptime_data = {}

        for h in host_list:
            try:
                metric_data = zbx.get_metric_data(
                    hostname=h,
                    metric_name=uptime_items.get(grp, "System uptime"),  # fallback default
                    time_from=time_from,
                    time_to=time_to
                )

                df = pd.DataFrame(metric_data['data'])
                uptime = calculate_uptime(df)
                uptime_data[h] = (uptime[0], uptime[1], uptime[2])
            except Exception as e:
                # Handle missing data or other errors gracefully
                uptime_data[h] = (0, 0, 0)

        # Convert collected uptime data to DataFrame
        df_uptime = pd.DataFrame.from_dict(
            uptime_data, orient='index',
            columns=['Uptime', 'Downtime', 'Availability (%)']
        ).reset_index().rename(columns={'index': 'Host Name'})

        # Add IPs
        df_uptime['Host IP'] = df_uptime['Host Name'].apply(
            lambda h: zbx.get_host_ip(h).get('data', {}).get('ip', 'N/A')
        )
        df_uptime = df_uptime[['Host Name', 'Host IP', 'Uptime', 'Downtime', 'Availability (%)']]
        
        grouped_uptime_data[grp] = df_uptime
    report_file = generate_excel_report(Beginning=unixtotime(time_from), Span=seconds_to_compact_time(time_to-time_from), report_type=report_type, df_dict=grouped_uptime_data)

    return report_file

def main_f(time_from: int, time_to: int, hostname: Optional[List[str]] = None, metric_name: Optional[List[str]] = None, to_email: Optional[List[str]] = None, report_type = None, user_name = None, group: Optional[List[str]] = None, client_group: Optional[List[str]] = None):
    ...
    if report_type == 'interface':
    ...
    elif report_type == 'Avalibility':
        # New logic for overview report (from main_t)
        report_data = get_overview_report(
            time_from=time_from,
            time_to=time_to,
            report_type=report_type,
            group=group,
        ...
        email_sender.send_email_with_attachment(
            ...
            subject=f"Overview Report: {report_type} from {unixtotime(time_from)} to {unixtotime(time_to)}",
            body=f"Please find the attached overview report for the period from {unixtotime(time_from)} to {unixtotime(time_to)}.",
